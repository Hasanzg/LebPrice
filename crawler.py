"""
Standalone scraper that saves to Django database.
Can run independently: python crawler.py
"""

import os
import sys
import django
import requests
import time
from bs4 import BeautifulSoup
import re
from concurrent.futures import ThreadPoolExecutor, as_completed
from threading import Lock
from decimal import Decimal, InvalidOperation
from datetime import datetime

# Django setup - Using 'Project' as project name
os.environ.setdefault('DJANGO_SETTINGS_MODULE', 'Project.settings')

# Add the parent directory to the path if needed
sys.path.append(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))

try:
    django.setup()
    from products.models import Product, Category, PriceHistory
    from django.utils import timezone
    from django.db import transaction, OperationalError
    USE_DJANGO = True
    print("Django database connection established")
except Exception as e:
    USE_DJANGO = False
    print(f"Django not available, will save to CSV only: {e}")

BASE_API = "https://pcandparts.com/wp-json/wc/store/products"
STORE_NAME = "PC and Parts"  # Store identifier

CATEGORIES = [
    "computer-cases", "cooling", "cpu", "ram", "motherboard",
    "power-supplies", "storage", "video-card", "home-tv-monitor",
    "camera", "ipad", "ipod", "mobile-phone", "tablet", "watch",
    "barcode-reader", "flash-memory", "keyboard-mouse", "monitor",
    "keyboard", "Headset", "speaker", "access-point", "desktops",
    "laptops", "accessories", "software"
]

# Headers to dynamically load site and get the actual products 
HEADERS = {
    "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) "
                  "AppleWebKit/537.36 (KHTML, like Gecko) "
                  "Chrome/130.0 Safari/537.36"
}

print_lock = Lock()
stats = {
    'total_fetched': 0,
    'db_created': 0,
    'db_updated': 0,
    'db_errors': 0,
    'db_skipped': 0
}

def safe_print(message):
    """Thread-safe printing."""
    with print_lock:
        print(message)

def clean_html(text):
    """Remove HTML tags and clean up text."""
    if not text:
        return None
    soup = BeautifulSoup(text, "html.parser")
    cleaned = soup.get_text(separator=" ", strip=True)
    cleaned = re.sub(r'\s+', ' ', cleaned)
    return cleaned.strip()

def extract_price(price_html):
    """Extract numeric price from HTML price string."""
    if not price_html:
        return None
    clean = clean_html(price_html)
    match = re.search(r'[\d,]+\.?\d*', clean)
    if match:
        price_str = match.group(0).replace(',', '')
        try:
            return Decimal(price_str)
        except InvalidOperation:
            return None
    return None

def save_to_django_db(product_data, category_name):
    """Save product to Django database with retry logic and duplicate prevention."""
    if not USE_DJANGO:
        return None, False
    
    max_retries = 5
    retry_delay = 0.5
    
    for attempt in range(max_retries):
        try:
            with transaction.atomic():
                # Get or create category
                category, _ = Category.objects.get_or_create(
                    name=category_name,
                    defaults={'slug': category_name.lower().replace(' ', '-')}
                )
                
                # Convert stock status
                stock_status = 'in_stock' if product_data['stock_status'] == 'In Stock' else 'out_of_stock'
                
                # Check if product already exists using product_id and store_name
                # This prevents duplicates based on unique combination
                try:
                    existing_product = Product.objects.get(
                        product_id=product_data['product_id'],
                        store_name=STORE_NAME
                    )
                    old_price = existing_product.price
                    created = False
                except Product.DoesNotExist:
                    old_price = None
                    created = True
                
                # Update or create product with all required fields
                product, was_created = Product.objects.update_or_create(
                    product_id=product_data['product_id'],
                    store_name=STORE_NAME,  # Part of unique constraint
                    defaults={
                        'sku': product_data.get('sku'),
                        'category': category,
                        'product_name': product_data['product_name'],
                        'description': product_data.get('description'),
                        'price': product_data.get('price'),
                        'currency': product_data.get('currency', 'USD'),
                        'stock_status': stock_status,
                        'product_url': product_data['product_url'],
                        'image_url': product_data.get('image_url'),
                        'last_scraped': timezone.now()
                    }
                )
                
                # Track price changes only if product was updated (not created)
                if not created and old_price and product_data.get('price'):
                    if old_price != product_data['price']:
                        PriceHistory.objects.create(
                            product=product,
                            price=old_price,
                            currency=product.currency,
                            stock_status=product.stock_status
                        )
                
                # Update stats
                with print_lock:
                    if was_created:
                        stats['db_created'] += 1
                    else:
                        stats['db_updated'] += 1
                
                return product, was_created
            
        except OperationalError as e:
            if 'database is locked' in str(e) and attempt < max_retries - 1:
                time.sleep(retry_delay * (attempt + 1))  # Exponential backoff
                continue
            else:
                with print_lock:
                    stats['db_errors'] += 1
                safe_print(f"  [DB Error] {product_data.get('product_id')}: {e}")
                return None, False
                
        except Exception as e:
            with print_lock:
                stats['db_errors'] += 1
            safe_print(f"  [DB Error] {product_data.get('product_id')}: {e}")
            return None, False
    
    # If all retries failed
    with print_lock:
        stats['db_errors'] += 1
    return None, False

def fetch_page(category, page):
    """Fetch a single page of products."""
    url = f"{BASE_API}?category={category}&page={page}"
    
    try:
        r = requests.get(url, headers=HEADERS, timeout=15)
        
        if r.status_code != 200:
            return None
        
        data = r.json()
        
        if not data or len(data) == 0:
            return None
        
        products = []
        for item in data:
            image_url = None
            if item.get('images') and len(item['images']) > 0:
                image_url = item['images'][0].get('src')
            
            price_html = item.get('price_html', '')
            price = extract_price(price_html)
            description = clean_html(item.get('short_description', ''))
            stock_status = 'In Stock' if item.get('is_in_stock') else 'Out Of Stock'
            
            product_data = {
                "store_name": STORE_NAME,
                "category": category,
                "product_name": item.get('name'),
                "price": price,
                "currency": "USD",
                "stock_status": stock_status,
                "product_url": item.get('permalink'),
                "image_url": image_url,
                "description": description,
                "product_id": item.get('id'),
                "sku": item.get('sku')
            }
            
            # Save to Django DB immediately
            if USE_DJANGO:
                save_to_django_db(product_data, category)
            
            products.append(product_data)
        
        with print_lock:
            stats['total_fetched'] += len(products)
        
        return products
        
    except Exception as e:
        safe_print(f"[{category}] Error on page {page}: {e}")
        return None

def get_category_products(category, max_workers=1):
    """Fetch all products from a category using multithreading."""
    safe_print(f"Starting category: {category}")
    
    all_products = []
    page = 1
    
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        futures = []
        
        while True:
            future = executor.submit(fetch_page, category, page)
            futures.append((page, future))
            page += 1
            
            # Check first page to see if category exists
            if page == 2:
                result = futures[0][1].result()
                if result is None:
                    safe_print(f"[{category}] No products found")
                    return []
            
            # Fetch pages in batches
            if len(futures) >= max_workers * 2:
                break
        
        # Continue fetching until no more results
        while futures or page < 100:
            # Submit new page requests
            while len(futures) < max_workers * 2 and page < 100:
                future = executor.submit(fetch_page, category, page)
                futures.append((page, future))
                page += 1
            
            # Process completed futures
            completed = []
            for i, (page_num, future) in enumerate(futures):
                if future.done():
                    result = future.result()
                    if result is None:
                        page = 100  # Stop pagination
                    else:
                        all_products.extend(result)
                        safe_print(f"[{category}] Page {page_num}: {len(result)} products")
                    completed.append(i)
            
            # Remove completed futures
            for i in sorted(completed, reverse=True):
                page_num, future = futures.pop(i)
                if future.result() is None:
                    page = 100
            
            if not futures:
                break
            
            time.sleep(0.1)
    
    safe_print(f"[{category}] ✓ Total: {len(all_products)} products")
    return all_products

def scrape_all_categories(max_workers=1): 
    """Scrape all categories sequentially to avoid database locks."""
    all_products = []
    
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        future_to_category = {
            executor.submit(get_category_products, cat, max_workers=1): cat 
            for cat in CATEGORIES
        }

        for future in as_completed(future_to_category):
            category = future_to_category[future]
            try:
                products = future.result()
                all_products.extend(products)
            except Exception as e:
                safe_print(f"[{category}] Failed: {e}")
    
    return all_products

def save_to_csv(products):
    """Save products to CSV as backup."""
    if not products:
        return
    
    try:
        import pandas as pd
        
        df = pd.DataFrame(products)
        
        column_order = [
            'product_id', 'store_name', 'sku', 'category', 'product_name',
            'price', 'currency', 'stock_status', 'product_url',
            'image_url', 'description'
        ]
        df = df[column_order]
        
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        filename = f"pcandparts_products_{timestamp}.csv"
        
        df.to_csv(filename, index=False, encoding='utf-8')
        df.to_csv("pcandparts_products_latest.csv", index=False, encoding='utf-8')
        
        print(f"CSV saved: {filename}")
        
    except Exception as e:
        print(f"Error saving CSV: {e}")

def main():
    start_time = time.time()
    
    print("="*60)
    print("PC and Parts Scraper - Django Compatible")
    print("="*60)
    print(f"Store: {STORE_NAME}")
    print(f"Database: {'Django ✓' if USE_DJANGO else 'CSV only'}")
    print(f"Categories: {len(CATEGORIES)}")
    print(f"Workers: 1 category at a time (SQLite safe)")
    print("="*60)
    print()
    
    # Scrape all products
    all_products = scrape_all_categories(max_workers=1)
    
    elapsed = time.time() - start_time
    
    print()
    print("="*60)
    print("Scraping Completed!")
    print("="*60)
    print(f"Products fetched: {stats['total_fetched']}")
    
    if USE_DJANGO:
        print(f"Database - Created: {stats['db_created']}")
        print(f"Database - Updated: {stats['db_updated']}")
        print(f"Database - Errors: {stats['db_errors']}")
        print(f"Total in DB: {stats['db_created'] + stats['db_updated']}")
    
    print(f"Time elapsed: {elapsed:.2f} seconds")
    print("="*60)
    
    # Save CSV as backup
    if all_products:
        save_to_csv(all_products)
    
    return all_products

if __name__ == "__main__":
    main()